export const SEARCH_STRATEGIES = Object.freeze({
  ...
  // The two strategies above are an experiment and its control, described in
  // docs/UNBIASED_SEARCH_EXPERIMENT.md: one ranks with price per square metre,
  // the other is deliberately blind to it, so the shortlists can be compared.
  // Neither is edited here, because changing an arm mid-experiment makes every
  // run before the change incomparable with every run after it.
  //
  // This is a third arm. It ranks on how far a property is priced below
  // comparable stock in its own neighbourhood, against the measured exit price
  // there. On the run of 22 August 2026, the discount to the area was the only
  // thing that separated the deals that cleared break-even from the ones that
  // did not: none of the 58 properties priced at or above their area cleared
  // it, against 40 per cent of those 15 to 30 per cent below.
  deal_quality: Object.freeze({
    id: 'deal_quality',
    scoringMode: 'deal_quality_v1',
    idealistaSortBy: 'mostRecent',
    includeEconomicDoorSignals: false,
    shortlistTieBreaker: 'deal_margin',
  }),
