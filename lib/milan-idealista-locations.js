...
/**
 * Every neighbourhood that resolves to an exact Idealista location id, as area
 * names the pipeline accepts.
 *
 * A run that asks for "Milano" gets one city-wide sweep ordered by recency, and
 * recency follows listing density, which follows price. On the run of
 * 22 August 2026 that put 33 listings in Città Studi at -20.9 per cent average
 * modelled ROI, 23 in Sempione at -32.6 and 15 in Moscova at -52.3, while
 * Chiesa Rossa got 11 at +49.6. Naming the neighbourhoods gives each one its
 * own budget instead of letting the market's distribution spend it.
 */
export const MILAN_IDEALISTA_AREA_NAMES = MILAN_IDEALISTA_NEIGHBORHOODS
  .map((location) => location.idealista_neighborhood_name);
