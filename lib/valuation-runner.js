...
import { assessCandidates, dealRankingScore } from './deal-ranking.js';
...
/**
 * The measured exit benchmark, if one has been built.
 *
 * Its absence is not an error: a run in a city nobody has collected exit prices
 * for still ranks, on the discount to the area alone, and every property comes
 * back marked unassessable so nobody mistakes the ranking for a return.
 */
async function loadExitBenchmark(log = () => {}) {
  try {
    const raw = await fs.readFile(
      new URL('../config/valuation-profiles/milan-exit-benchmark-v1.json', import.meta.url), 'utf8');
    return JSON.parse(raw);
  } catch {
    log('No exit benchmark found: deals will be ranked without a measured exit price');
    return null;
  }
}

export function sourceCandidateOrder(strategy) {
  // Ordering the pool decides which properties are valued at all, not merely
  // how they are displayed. The deal-quality arm cannot express its order in
  // SQL - the discount is computed against the pool once the pool exists - so
  // it asks for a stable, opinion-free order and ranks afterwards. Ordering it
  // by door score would spend the valuation budget worst-first: on the run of
  // 22 August 2026 that score correlated with modelled ROI at -0.182.
  if (strategy.id === 'deal_quality') return 'id.asc';
...
export function scoreValuedResult(strategy, doorEngine, spread, gptAnalysis, dealQuality = null) {
  // The deal-quality arm ranks on the assessment the caller computed against
  // the property's own neighbourhood. A property whose area could not be
  // measured is not a failed one, and dealRankingScore places it accordingly.
  if (strategy.id === 'deal_quality') {
    if (!doorEngine?.fractioningFeasible) return 0;
    return dealRankingScore(dealQuality);
  }
...
export async function runValuationFromSupabase(options = {}) {
  ...
  // The deal-quality arm needs the whole pool before it can rank any of it:
  // the discount is measured against the other properties the run found in the
  // same neighbourhood. Fetching rows is cheap and valuing them is not, so it
  // reads everything eligible and then chooses which to value.
  const rankingNeedsWholePool = strategy.id === 'deal_quality';
  const sourceFetchLimit = rankingNeedsWholePool
    ? 10000
    : Math.min(10000, Math.max(limit, limit * 2));
  const sourceCandidates = await fetchSupabasePages(supabaseRest, `triage_source_listings?${query}`, sourceFetchLimit);

  let candidates;
  let dealQualityByCandidate = new Map();
  if (rankingNeedsWholePool) {
    const pool = dedupeValuationCandidates(sourceCandidates);
    const benchmark = await loadExitBenchmark(log);
    const assessed = assessCandidates(pool, { benchmark });
    dealQualityByCandidate = new Map(assessed.map((entry) => [entry.candidate, entry.assessment]));
    candidates = assessed
      .sort((left, right) => dealRankingScore(right.assessment) - dealRankingScore(left.assessment))
      .slice(0, limit)
      .map((entry) => entry.candidate);
  } else {
    candidates = dedupeValuationCandidates(sourceCandidates, limit);
  }
  ...
  for (const [index, sourceRow] of candidates.entries()) {
    ...
    results.push({
      ...
      ranking_score: scoreValuedResult(strategy, doorEngine, spread, gptAnalysis,
        dealQualityByCandidate.get(sourceRow) ?? null),
      // Why this property is ranked where it is, next to the ranking itself.
      deal_quality: dealQualityByCandidate.get(sourceRow) ?? null,
