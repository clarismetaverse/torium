...
export async function runValuationFromSupabase(options = {}) {
  ...
  const provider = valuationMode === 'ai'
    ...
    : { provider: 'deterministic_market_profile', model: 'milan_microzone_exit_v1_2026_06' };
  ...
  const resultLinks = await saveResults(supabaseRest, run, sorted, provider, valuationMode);
  return {
    ok: true,
    run_id: runId,
    search_strategy: strategy.id,
