  ...
  properties: {
    final_unit_plan: {
      ...
      items: {
        ...
        properties: {
          unit_type: { type: 'string', enum: ['bilocale', 'trilocale', 'unknown'] },
...
async function saveResults(supabaseRest, run, results, provider) {
...
export async function runValuationFromSupabase(options = {}) {
  ...
  const limit = Math.max(1, Math.min(20, numberOption(options.limit ?? env.TORIUM_GPT_TRIAGE_LIMIT, 5)));
  ...
  const provider = resolveValuationProvider(env);
  ...
  for (const [index, sourceRow] of candidates.entries()) {
    ...
    const gptAnalysis = await analyzeListing({ provider, prompt, sourceRow, listing, doorEngine, investorProfile, fetchImpl });
  ...
  const resultLinks = await saveResults(supabaseRest, run, sorted, provider);
  return {
    ok: true,
    run_id: runId,
    search_strategy: strategy.id,
    ...
    valuation_provider: provider.provider,
    valuation_model: provider.model,
    gpt_analyzed_count: sorted.length,
