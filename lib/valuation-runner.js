...
import { assessInvestment, compareInvestments } from './investment-assessment.js';
import { BENCHMARK_VERSION } from './asking-price-benchmark.js';
...
import { mergeUnifiedProperty, summarizePriceDifferences } from './source-offers.js';
...
async function fetchSupabasePages(supabaseRest, pathname, limit, pageSize = 1000) {
  const rows = [];
  ...
  while (rows.length < limit) {
    ...
    const page = await supabaseRest(`${basePath}?${params.toString()}`);
...
export function dedupeValuationCandidates(rows, limit = rows.length) {
  const groups = new Map();
  for (const row of rows) {
    const key = row.canonical_source_key || `${row.source_channel}:${row.source_listing_id || row.source_url || row.source_fingerprint || row.id}`;
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(row);
  }
  return [...groups.values()].slice(0, limit).map((members) => mergeUnifiedProperty(members));
}
...
export function sourceCandidateOrder(strategy) {
  if (strategy.id === 'deal_quality_v2') return 'id.asc';
...
export function sortValuedResults(results, strategy) {
  if (strategy.id === 'deal_quality_v2') return [...results].sort(compareInvestments);
...
export function sourceRowToListing(row) {
  const raw = row.raw_listing && typeof row.raw_listing === 'object' ? row.raw_listing : {};
  const rawPrice = typeof raw.price === 'number' || typeof raw.price === 'string' ? raw.price : null;
  return {
    ...raw,
    ...
    floor: normalizeItalianFloor(row.floor ?? raw.floor ?? null),
    ...
    source_offers: row.source_offers ?? [],
    price_comparison: row.price_comparison ?? null,
    raw,
  };
}

export function prepareInvestmentCandidates(rows, investorProfile, benchmark, now) {
  return dedupeValuationCandidates(rows).map(sourceRow => {
    const listing = sourceRowToListing(sourceRow);
    const doorEngine = runDoorEngine(listing, investorProfile, { includeEconomicSignals: false, scoringMode: 'physical_evidence_v2' });
    return { ...sourceRow, investment_assessment: assessInvestment({ listing, doorEngine, benchmark, now }) };
  }).sort(compareInvestments);
}

function investmentValuation(assessment, door) {
  const valued = assessment.status === 'assessed';
  const units = door.plannedUnitMix.map((u, i) => ({ ...u,
    sale_value_low_eur: valued ? u.estimated_size_mq * assessment.unit_references[i].p25_eur_mq : null,
    sale_value_base_eur: valued ? u.estimated_size_mq * assessment.unit_references[i].median_eur_mq : null,
    sale_value_high_eur: valued ? u.estimated_size_mq * assessment.unit_references[i].p75_eur_mq : null,
  }));
  return { final_unit_plan: units,
    total_sale_value_low_eur: assessment.underwriting?.scenarios.low.exitValueEur ?? null,
    total_sale_value_base_eur: assessment.underwriting?.scenarios.base.exitValueEur ?? null,
    total_sale_value_high_eur: assessment.underwriting?.scenarios.high.exitValueEur ?? null,
    fractioning_confidence: 'low', valuation_confidence: valued ? 'medium' : 'low',
    positive_signals: [], red_flags: ['Prezzi richiesti, non prezzi di compravendita. Fattibilità tecnica e legale da verificare.'],
    missing_information: valued ? [] : ['Comparabili recenti ristrutturati insufficienti per il mix di unità previsto.'],
    human_due_diligence_questions: ['Verificare planimetria, vincoli e prezzi di compravendita della microzona.'],
    recommended_action: 'manual_review', valuation_method: 'unit_mix_asking_underwriting_v2' };
}
...
function buildResultLink(result, rank) {
  return {
    ...
    source_offers: result.source_offers ?? result.source_row?.source_offers ?? [],
    price_comparison: result.price_comparison ?? result.source_row?.price_comparison ?? null,
  };
}
...
function buildPropertyRow(result, rank, run) {
  ...
  return {
    ...
    estimated_project_cost_eur: underwriting.costs?.projectCostEur ?? door.estimatedProjectCost ?? source.estimated_project_cost_eur ?? null,
...
export function buildCompactRunRawOutput(run, summary) {
  const existing = run.raw_output && typeof run.raw_output === 'object' ? run.raw_output : {};
  const { results: _duplicatedResults, result_links: _duplicatedLinks, ...compactExisting } = existing;
  return {
    ...compactExisting,
    ...summary,
  };
...
async function saveResults(supabaseRest, run, results, provider, valuationMode, log = console.log) {
  const existingRows = await fetchSupabasePages(
    supabaseRest,
    `triage_properties?run_id=eq.${encodeURIComponent(run.run_id)}&select=id&order=id.asc`,
    10000,
    500,
  );
  const deleteBatchSize = 10;
  for (let offset = 0; offset < existingRows.length; offset += deleteBatchSize) {
    const ids = existingRows
      .slice(offset, offset + deleteBatchSize)
      .map((row) => String(row.id || '').trim())
      .filter(Boolean);
    if (!ids.length) continue;
    await supabaseRest(
      `triage_properties?run_id=eq.${encodeURIComponent(run.run_id)}&id=in.(${ids.map(encodeURIComponent).join(',')})`,
      {
        method: 'DELETE',
        headers: { Prefer: 'return=minimal' },
      },
    );
    log(`Cleared previous valuation rows ${Math.min(offset + deleteBatchSize, existingRows.length)}/${existingRows.length}`);
  }
  log(`Cleared previous valuation rows for ${run.run_id}`);
  ...
  for (let offset = 0; offset < rows.length; offset += 250) {
    ...
    log(`Saved valuation rows ${Math.min(offset + 250, rows.length)}/${rows.length}`);
  }
  ...
  const rawOutput = buildCompactRunRawOutput(run, {
    ...
    price_comparison_summary: summarizePriceDifferences(results),
  });
  log(`Saving compact valuation summary for ${run.run_id}`);
  ...
  log(`Saved compact valuation summary for ${run.run_id}`);
...
export async function runValuationFromSupabase(options = {}) {
  ...
  const maxLimit = valuationMode === 'deterministic' ? 5000 : 20;
  ...
  const investmentMode = strategy.id === 'deal_quality_v2';
  if (investmentMode && valuationMode !== 'deterministic') throw Error('deal_quality_v2 requires deterministic valuation');
  if (investmentMode && minDoorScore !== 0) throw Error('deal_quality_v2 cannot prefilter by a historical door score');
  const sourceFetchLimit = investmentMode ? 10001 : Math.min(10000, Math.max(limit, limit * 2));
  const sourceCandidates = await fetchSupabasePages(supabaseRest, `triage_source_listings?${query}`, sourceFetchLimit);
  if (investmentMode && sourceCandidates.length > 10000) throw Error('Investment pool exceeds supported size; no partial-pool ranking allowed');
  ...
  if (investmentMode && !benchmark && env.TORIUM_EXIT_BENCHMARK_PATH) benchmark = JSON.parse(await fs.readFile(env.TORIUM_EXIT_BENCHMARK_PATH, 'utf8'));
  if (investmentMode && benchmark?.version !== BENCHMARK_VERSION) throw Error('A reviewed asking_comparables_v2 benchmark is required');
  const assessedPool = investmentMode ? prepareInvestmentCandidates(sourceCandidates, investorProfile, benchmark, options.now) : null;
  const candidates = investmentMode ? assessedPool.filter(row => row.investment_assessment.status !== 'not_fractionable').slice(0, limit) : dedupeValuationCandidates(sourceCandidates, limit);
  ...
  const preview = {
    ...
    ...(investmentMode ? { ranking_basis: 'modelled_roi_pct', assessed_pool_count: assessedPool.filter(r => r.investment_assessment.status === 'assessed').length,
      unassessable_pool_count: assessedPool.filter(r => r.investment_assessment.status === 'unassessable').length,
      not_fractionable_pool_count: assessedPool.filter(r => r.investment_assessment.status === 'not_fractionable').length,
      benchmark_hash: benchmark.input_sha256 } : {}),
  };
  ...
  const provider = valuationMode === 'ai'
    ...
    : { provider: investmentMode ? 'asking_comparables' : 'deterministic_market_profile', model: investmentMode ? benchmark.version : deterministicProfile.version };
  const results = [];
  ...
  for (const [index, sourceRow] of candidates.entries()) {
    ...
    const gptAnalysis = investmentMode ? investmentValuation(sourceRow.investment_assessment, doorEngine) : valuationMode === 'ai'
    ...
    const underwriting = (investmentMode && sourceRow.investment_assessment.underwriting) || calculateUnderwriting({
      purchasePriceEur: listing.price,
      purchaseCostRate: doorEngine.purchaseCostRate,
      purchaseCostsEur: doorEngine.purchaseCosts,
      existingUnits: doorEngine.existingUnits,
      finalUnits: doorEngine.estimatedFinalUnits,
      newUnitsCreated: doorEngine.newUnitsCreated,
      costPerFinalUnitEur: doorEngine.costPerFinalUnit,
      costPerTrilocaleEur: doorEngine.costPerTrilocale,
    ...
    results.push({
      ...
      source_offers: sourceRow.source_offers ?? [],
      price_comparison: sourceRow.price_comparison ?? null,
      ranking_score: investmentMode ? sourceRow.investment_assessment.ranking_score : scoreValuedResult(strategy, doorEngine, spread, gptAnalysis),
      ...(investmentMode ? { investment_assessment: sourceRow.investment_assessment, ranking_basis: 'modelled_roi_pct' } : {}),
  ...
  const { links: resultLinks, valuedCount } = await saveResults(supabaseRest, run, sorted, provider, valuationMode, log);
  return {
    ok: true,
    run_id: runId,
    search_strategy: strategy.id,
