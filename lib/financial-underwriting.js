export const UNDERWRITING_VERSION = 'max_doors_20k_v1';
...
export function calculateUnderwriting(input = {}, assumptions = {}) {
  ...
  const scenario = (exitValueEur) => {
    if (exitValueEur === null || projectCostEur === null || projectCostEur <= 0) {
      return { exitValueEur, profitLossEur: null, roiPct: null, marginOnSalesPct: null };
    }
    const profitLossEur = roundMoney(exitValueEur - projectCostEur);
    return {
      exitValueEur: roundMoney(exitValueEur),
      profitLossEur,
      roiPct: roundPercent((profitLossEur / projectCostEur) * 100),
  ...
  return {
    ...
    assumptions: { purchaseCostRate, costPerNewUnitEur },
    costs: {
    ...
    formulas: {
      ...
      profitLoss: 'exit_value - project_cost',
      roi: '(profit_loss / project_cost) * 100',
