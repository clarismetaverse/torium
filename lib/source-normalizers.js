...
function normalizeCondition(raw) {
  ...
  if (['toberenovated', 'to be renovated', 'daristrutturare', 'da ristrutturare', 'renew'].some((term) => normalized.includes(normalizeText(term)))) return 'renew';
  if (['newconstruction', 'nuova costruzione', 'new'].some((term) => normalized.includes(normalizeText(term)))) return 'newconstruction';
  if (['excellent', 'ottimo'].some((term) => normalized.includes(normalizeText(term)))) return 'excellent';
  if (['good', 'buono'].some((term) => normalized.includes(normalizeText(term)))) return 'good';
  return value;
}
...
export function normalizeSourceListing(raw, context = {}) {
  ...
  const areaLabel = firstString(context.query_area, context.area, raw.area, district, neighborhood);
