...
import { normalizeListingV1 } from './normalized-listing-v1.js';
...
function categorizeFeatures(features, raw = {}) {
  ...
  const renovation_features = {
    ...
    has_garden: firstBoolean(raw.garden, raw.hasGarden, raw.privateGarden, raw.topology?.garden) === true || featureIncludes(features, ['giardino', 'garden', 'parco privato', 'terreno']),
    has_pool: firstBoolean(raw.pool, raw.swimmingPool, raw.hasSwimmingPool, raw.topology?.pool) === true || featureIncludes(features, ['piscina', 'swimming pool']),
    has_garage: firstBoolean(raw.garage, raw.hasGarage, raw.parkingSpace, raw.topology?.garage) === true || featureIncludes(features, ['garage', 'box auto', 'posto auto', 'parcheggio']),
    panoramic_view: firstBoolean(raw.seaViews, raw.hasSeaViews, raw.lakeView) === true || featureIncludes(features, ['vista lago', 'vista mare', 'vista panoramica', 'panoramic']),
  };
...
export function normalizeSourceListing(raw, context = {}) {
  ...
  const normalizedV1 = ['idealista', 'immobiliare'].includes(String(sourceChannel).toLowerCase())
    ? normalizeListingV1(raw, { ...context, source_channel: sourceChannel })
    : null;

  const url = firstString(
    normalizedV1?.canonical_url,
    raw.url,
    raw.link,
    raw.detailUrl,
    raw.seoUrl,
  ...
  const sourceListingId = firstString(
    normalizedV1?.source_listing_id,
    raw.propertyCode,
    raw.id,
    raw.uuid,
    raw.adId,
  ...
  const address = firstString(
    normalizedV1?.address?.formatted,
    raw.address,
    raw.street,
    raw.location,
  ...
  const observedCity = firstString(raw.municipality, raw.city, raw.comune, raw.geography?.municipality?.name);
  const city = firstString(normalizedV1?.location?.city, observedCity, context.query_municipality, context.municipality);
  const district = firstString(normalizedV1?.location?.source_macrozone, raw.district, raw.zone, raw.zona, getFirstPath(raw, ['location.zone', 'realEstate.properties.0.location.zone']));
  const neighborhood = firstString(normalizedV1?.location?.source_microzone, raw.neighborhood, raw.microzone, raw.quartiere, getFirstPath(raw, ['location.microzone', 'realEstate.properties.0.location.microzone.name']));
  const sourceAreaLabel = firstString(neighborhood, raw.area, district);
  const canonicalZone = normalizedV1?.location?.canonical_zone_id ? {
    id: normalizedV1.location.canonical_zone_id,
    name: normalizedV1.location.canonical_zone_name,
  } : canonicalMilanAreaLabel({
    neighborhood,
    district,
    area: sourceAreaLabel,
    query_area: context.query_area,
    title,
    address,
  });
  const areaLabel = canonicalZone?.name || sourceAreaLabel;

  const price = normalizedV1 ? normalizedV1.asking_price.amount_eur : firstNumber(
    raw.price,
    raw.priceValue,
    raw.price_value,
    raw.priceInfo?.price,
  ...
  const size = normalizedV1 ? normalizedV1.surface.value_sqm : firstNumber(
    raw.size,
    raw.surface,
    raw.surfaceMq,
    raw.surface_mq,
  ...
  const priceByArea = normalizedV1 ? normalizedV1.asking_price.price_per_sqm_eur ||
    (normalizedV1.asking_price.amount_eur && normalizedV1.surface.value_sqm
      ? Math.round(normalizedV1.asking_price.amount_eur / normalizedV1.surface.value_sqm)
      : null) : firstNumber(
    raw.priceByArea,
    raw.pricePerSqm,
  ...
  const rooms = normalizedV1?.rooms ?? firstNumber(raw.rooms, raw.locali, raw.roomsNumber, raw.features?.rooms, deepFindNumber(raw, ['rooms', 'locali']));
  const bathrooms = normalizedV1?.bathrooms ?? firstNumber(raw.bathrooms, raw.bagni, raw.features?.bathrooms, deepFindNumber(raw, ['bathrooms', 'bagni']));
  const floor = normalizedV1?.floor?.normalized ?? normalizeItalianFloor(firstString(raw.floor, raw.floorNumber, raw.features?.floor, deepFindString(raw, ['floor', 'piano'])));

  const hasLift = normalizedV1 ? normalizedV1.has_lift : firstBoolean(raw.hasLift, raw.lift, raw.elevator, raw.ascensore, deepFindBoolean(raw, ['lift', 'elevator', 'ascensore']));
  const hasPlan = normalizedV1 ? normalizedV1.media.has_floor_plan : extractHasPlan(raw);

  const latitude = normalizedV1?.location?.latitude ?? firstNumber(raw.latitude, raw.lat, raw.location?.latitude, raw.location?.lat, getFirstPath(raw, ['realEstate.properties.0.location.latitude']));
  const longitude = normalizedV1?.location?.longitude ?? firstNumber(raw.longitude, raw.lng, raw.lon, raw.location?.longitude, raw.location?.lng, getFirstPath(raw, ['realEstate.properties.0.location.longitude']));
  const thumbnail = normalizedV1?.media?.thumbnail_url || extractImage(raw);
  ...
  const propertyType = firstString(normalizedV1?.property_type, raw.propertyType, raw.type, raw.category, raw.typology, deepFindString(raw, ['propertytype', 'typology', 'tipologia']));
  const condition = normalizedV1?.condition || normalizeCondition(raw);
  const features = normalizedV1?.features || normalizeFeatureArray(raw);
  const isNew = normalizedV1 ? normalizedV1.is_new_construction : firstBoolean(raw.isNew, raw.new, raw.newConstruction, raw.is_new, raw.is_new_construction);
  const { renovation_features, ignored_features, risk_features } = categorizeFeatures(features, raw);
  ...
  const listing = {
    ...
    source_area_label: sourceAreaLabel,
    query_area: context.query_area || context.area || null,
    price,
    priceStatus: normalizedV1?.asking_price?.status || (price ? 'known' : 'missing'),
    priceByArea,
    size,
    rooms,
    bathrooms,
    ...
    longitude,
    thumbnail,
    photos: normalizedV1?.media?.images || [],
    floor_plans: normalizedV1?.media?.floor_plans || [],
    description,
    features,
    isNew,
    renovation_features,
  ...
  return {
    ...
    source_area_label: sourceAreaLabel,
    price_eur: price,
    price_status: normalizedV1?.asking_price?.status || (price ? 'known' : 'missing'),
    ...
    photos: normalizedV1?.media?.images || [],
    floor_plans: normalizedV1?.media?.floor_plans || [],
    adapter_version: normalizedV1?.adapter_version || null,
    normalized_v1: normalizedV1,
    raw_listing: raw,
    location_is_inferred: normalizedV1?.location?.city_is_inferred ?? !observedCity,
    listing,
  };
}
