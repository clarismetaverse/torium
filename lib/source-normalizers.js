...
export function normalizeSourceListing(raw, context = {}) {
  ...
  const observedCity = firstString(raw.municipality, raw.city, raw.comune);
  const city = firstString(observedCity, context.query_municipality, context.municipality);
  ...
  const floor = normalizeItalianFloor(firstString(raw.floor, raw.floorNumber, raw.features?.floor, deepFindString(raw, ['floor', 'piano'])));
  ...
  return {
    ...
    raw_listing: raw,
    location_is_inferred: !observedCity,
    listing,
  };
}
