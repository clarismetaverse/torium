...
export function getVillaPreTriageExclusion(item, intent = 'renovation') {
  ...
  if (!hasAny(typology, ['villa', 'villetta', 'house', 'casa indipendente', 'casale', 'countryhouse', 'detached', 'bifamiliare', 'rustico'])) {
    reasons.push('villa_or_house_typology_not_confirmed');
  }
  const listing = item?.listing || item || {};
  const condition = normalizeText(item?.property_condition || listing.status || listing.condition);
  const qualityFlags = [
    ...(Array.isArray(item?.quality_flags) ? item.quality_flags : []),
    ...(Array.isArray(listing.qualityFlags) ? listing.qualityFlags : []),
  ];
  const visibleListingText = normalizeText([
    listing.title,
    listing.description,
    ...(Array.isArray(listing.features) ? listing.features : []),
  ].filter(Boolean).join(' '));
  const isNewConstruction = item?.is_new === true
    || listing.isNew === true
    || condition === 'newconstruction'
    || qualityFlags.includes('is_new_true')
    || hasAny(visibleListingText, ['nuova costruzione']);
  if (intent === 'renovation' && isNewConstruction) reasons.push('new_construction_excluded');
