...
function unitBand(profileSet, profile, sizeMq, unitType) {
  const bands = profile.unit_size_bands?.length ? profile.unit_size_bands : (profileSet.unit_size_bands || []);
...
export function buildDeterministicValuation({ profileSet, sourceRow, listing, doorEngine }) {
  ...
  const valuedUnits = plannedMix.map((plannedUnit, index) => {
    ...
    const band = unitBand(profileSet, profile, unitSizeMq, plannedUnit.unit_type);
    ...
    return {
      ...
      valuation_reasoning: `${profile.name}; benchmark ${Math.round(profile.base_exit_eur_mq)} EUR/mq × fattore dimensionale ${Number(band.multiplier).toFixed(2)}; unità ${index + 1}/${finalUnits}`,
    };
  });
  ...
  return {
    valuation_method: 'deterministic_milan_zone_unit_v3',
    valuation_profile_version: profileSet.version,
    ...
    valuation_assumptions: {
      ...
      market_sentiment: profile.market_sentiment ?? null,
    },
    ...
    valuation_confidence: profile.provisional_fallback ? 'low' : 'medium',
    positive_signals: [
      `Profilo di valutazione: ${profile.name}`,
      `Mese del benchmark: ${profileSet.effective_month}`,
      `Andamento di mercato: ${(profile.market_sentiment?.label || 'non disponibile').replaceAll('_', ' ')}`,
      `Unità finali stimate: ${finalUnits}`,
      ...(doorEngine?.residualStudioIncluded ? ['Previsto un monolocale residuo da validare tecnicamente.'] : []),
    ],
    red_flags: [
      'Il benchmark di uscita utilizza prezzi richiesti, non prezzi di compravendite confermate.',
      'Il benchmark dei prezzi richiesti per zona, il premio per i piccoli tagli e il rapporto di superficie vendibile sono ipotesi provvisorie.',
      ...(profile.provisional_fallback ? ['Non è stata riconosciuta una macrozona specifica di Milano: è stato utilizzato il benchmark cittadino generale.'] : []),
      ...(doorEngine?.residualStudioIncluded
        ? ['La fattibilità del monolocale residuo deve essere verificata da un tecnico: 28 mq è soltanto una soglia progettuale prudenziale.']
        : []),
    ],
    missing_information: [
      'Validazione tecnica della distribuzione finale delle unità e della superficie vendibile effettiva.',
      'Compravendite comparabili specifiche per l’immobile e perizia professionale.',
      'Budget completo della ristrutturazione oltre alla stima parametrica configurata per unità.',
    ],
    human_due_diligence_questions: [
      'La planimetria e gli impianti dell’edificio confermano il numero di unità ipotizzato?',
      'Quali compravendite recenti e concluse sono comparabili per metratura e stato delle unità previste?',
      'Qual è il budget completo per ristrutturazione e commercializzazione?',
    ],
