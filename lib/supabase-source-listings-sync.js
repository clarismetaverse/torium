import { extractSourceListingAssets } from './listing-assets.js';
...
function buildRunRow(output) {
  return {
    ...
    scoring_mode: output.scoring_mode,
...
function buildSourceRow(item, output, index) {
  const canonicalSourceKey = item.canonical_source_key || item.source_key || item.source_url || item.source_fingerprint || String(index);
  const queryScopedSourceKey = [
    item.source_channel,
    item.query_name,
    item.query_area,
    canonicalSourceKey,
  ...
  return {
    ...
    source_key: queryScopedSourceKey,
    ...
    idealista_location_id: item.idealista_location_id ?? null,
    idealista_zone_id: item.idealista_zone_id ?? null,
    idealista_zone_name: item.idealista_zone_name ?? null,
    idealista_neighborhood_name: item.idealista_neighborhood_name ?? null,
    title: item.title,
    ...
    renovation_features: item.renovation_features ?? {},
    ignored_features: item.ignored_features ?? [],
...
export async function syncSourceListingsRunToSupabase(output, sourceListings) {
  ...
  const rows = sourceListings.map((item, index) => buildSourceRow(item, output, index));
  ...
  const assetRowsByKey = new Map();
  for (const item of sourceListings) {
    for (const asset of extractSourceListingAssets(item)) {
      assetRowsByKey.set(asset.asset_key, {
        run_id: output.run_id,
        ...asset,
        cache_status: 'remote',
  ...
  const assetRows = [...assetRowsByKey.values()];
  for (const chunk of chunkArray(assetRows, 500)) {
  ...
  console.log(`Saved massive source run to Supabase: ${output.run_id} (${rows.length} source listings, ${assetRows.length} media links)`);
}
